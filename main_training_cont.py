...
from fms_fsdp.utils.checkpointing_utils import Checkpointer,has_checkpoints_in_save_path
...
def main(**kwargs):
    ...
    print(f"\n** local_rank: {local_rank} \n** rank: {rank} \n** world_size: {world_size}") # test later with -n 2 in bsub
    ...
    # get data loader
    has_ckpt_in_save_path=True

    if rank == 0:
        print("Constructing datasets...")
    if not cfg.use_dummy_dataset:
        '''
        Check if there are already any checkpoints in the ckpt_save_path:
        If exist, cont. training from that checkpoint, else cont. training from ckpt_load_path
        '''
        has_ckpt_in_save_path=has_checkpoints_in_save_path(cfg,rank)
        if rank == 0:
            print(f"\n== has_ckpt_in_save_path: {has_ckpt_in_save_path} \n")

        if has_ckpt_in_save_path:
    ...
    #== Update model, optimizer, etc. from the latest checkpoint in ckpt_load_path:
    ...
    if rank==0:
        print(f"\n== start_step: {start_step} tokens_seen: {tokens_seen} when model is loaded from checkpointer.load()!")
    
    if not has_ckpt_in_save_path:
        # TODO: when ckpt_load_path and ckpt_save_path are different but both from the same dataset. 
